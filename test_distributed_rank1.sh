...
ckpt_dir=/GLOBALFS/nsccgz_zgchen_3/lqy/checkpoints/vary_cf/node1  # directory to save checkpoints
...
perf_out_path=/GLOBALFS/nsccgz_zgchen_3/lqy/IncrCP_paper/experimental_results/distributed/cf_e0003_$check_freq/node1 # output path
batch_size=1024
...
if [ $incrcp = 1 ]; then
  ...
  python -m torch.distributed.launch --nproc_per_node=8 --nnodes=2 --node-rank=1 --master_addr=$master_addr --master_port=1234 \
